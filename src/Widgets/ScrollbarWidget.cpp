...
void ScrollbarWidget::onCaptured(glm::dvec2 cursorPosition) {
    if (!_handleRectangleWidget->intersect(cursorPosition)) {
        ...
        const glm::dvec2 offsetLocalPosition =
            -(localPosition + glm::dvec2(0.5, -0.5) * static_cast<glm::dvec2>(_size));
        const glm::dvec2 halfHandleSize =
            static_cast<glm::dvec2>(_handleRectangleWidget->getSize()) * 0.5;
        const double progress =
            (offsetLocalPosition.y - halfHandleSize.y) / (_size.y - halfHandleSize.y * 2.0);
...
void ScrollbarWidget::processDragging(glm::dvec2 dragOffset) {
    ...
    offset = std::clamp(offset, static_cast<double>(-_size.y + _handleRectangleWidget->getSize().y),
                        0.0);
