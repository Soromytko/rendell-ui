...
static TabBarWidget::CreateTabButtonDelegate s_defaultCreateTabButtonDelegate =
    [](std::wstring title) {
...
void TabBarWidget::setModel(StringListModelSharedPtr model) {
    ...
    if (_model != model) {
        ...
        if (_model) {
            _itemAddedConnectionId =
                _model->itemAdded.connect(this, &TabBarWidget::onModelItemAdded);
            _itemRemovedConnectionId =
                _model->itemRemoved.connect(this, &TabBarWidget::onModelItemRemoved);
            _itemChangedConnectionId =
                _model->itemChanged.connect(this, &TabBarWidget::onModelItemChanged);
        }
...
void TabBarWidget::selectButton(TabButtonWidget *button) {
    auto it = std::find_if(_buttons.begin(), _buttons.end(),
                           [=](const TabButtonWidgetSharedPtr &currentButton) {
                               return currentButton.get() == button;
                           });
...
void TabBarWidget::onModelItemRemoved(const std::wstring &item, size_t itemIndex) {
    ...
    if (_currentIndex >= itemIndex) {
        if (_buttons.size() == 0) {
            ...
            const int newIndex =
                std::clamp(_currentIndex + 1, 0, static_cast<int>(_buttons.size()) - 1);
